...
public class SpinningCircleGameDiadromos : MonoBehaviour
{
    ...
    public GameObject askisiDiadromos;
    ...
    void Update()
    {
        totalReps = askisiDiadromos.GetComponent<AskisiDiadromos>().Synolikesepanalipseis;

        rectTransform.Rotate(Vector3.forward * spinSpeed * Time.deltaTime);

        if (successfulReps >= totalReps)
        {
            askisiDiadromos.GetComponent<AskisiDiadromos>().isComplete = true;
        }

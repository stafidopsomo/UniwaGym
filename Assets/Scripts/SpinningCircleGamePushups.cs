...
public class SpinningCircleGamePushups : MonoBehaviour
{
    ...
    public GameObject askisiPushups;
    ...
    void Update()
    {
        totalReps = askisiPushups.GetComponent<AskisiPushups>().Synolikesepanalipseis;

        rectTransform.Rotate(Vector3.forward * spinSpeed * Time.deltaTime);

        if (successfulReps >= totalReps)
        {
            askisiPushups.GetComponent<AskisiPushups>().isComplete = true;
        }
